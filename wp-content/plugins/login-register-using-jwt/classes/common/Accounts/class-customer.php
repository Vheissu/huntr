...
class Customer {
	...
	public function __construct() {
		...
		$this->host_key  = $mj_util->mo_jwt_get_option( 'password' );
	}
	 ...
	 */
	public function add_jwt_application( $name, $app_name ) {
		global $mj_util;
		$url           = $this->host_name . '/moas/rest/application/addoauth';
		$customer_key  = $mj_util->mo_jwt_get_option( 'mo_jwt_admin_customer_key' );
		$scope         = $mj_util->mo_jwt_get_option( 'mo_jwt_' . $name . '_scope' );
		$client_id     = $mj_util->mo_jwt_get_option( 'mo_jwt_' . $name . '_client_id' );
		$client_secret = $mj_util->mo_jwt_get_option( 'mo_jwt_' . $name . '_client_secret' );
		if ( false !== $scope ) {
			$fields = array(
	...
	public function submit_contact_us( $email, $phone, $query, $send_config = true ) {
		...
		global $mj_util;
		wp_get_current_user();
		//$plugin_config          = $mj_util->export_plugin_config( true );
	...
	public function send_otp_token( $email = '', $phone = '', $send_to_email = true, $send_to_phone = false ) {
		...
		$api_key      = $this->default_api_key;
		$username     = $this->email;
		$phone        = $mj_util->mo_jwt_get_option( 'mo_oauth_admin_phone' );
	...
	public function mo_oauth_forgot_password( $email ) {
		...
		$customer_key = $mj_util->mo_jwt_get_option( 'mo_oauth_admin_customer_key' );

		/* The customer API Key provided to you */
		$api_key = $mj_util->mo_jwt_get_option( 'mo_oauth_admin_api_key' );
	...
	public function check_internet_connection() {
		return (bool) @fsockopen( 'login.xecurify.com', 443, $iErrno, $sErrStr, 5 ); // phpcs:ignore
	}
	...
	private function send_request( $additional_headers = false, $override_headers = false, $field_string = '', $additional_args = false, $override_args = false, $url = '' ) {
		...
		if ( is_wp_error( $response ) ) {
			...
			echo wp_kses( "Something went wrong: $error_message", \mo_jwt_get_valid_html() );
			exit();
		}
	...
	public function manage_deactivate_cache() {
		global $mj_util;
		$lk = $mj_util->mo_jwt_get_option( 'mo_api_authentication_lk' );
		if ( ! $mj_util->mo_jwt_is_customer_registered() || false === $lk || empty( $lk ) ) {
			return;
		}
		$url          = $this->host_name . '/moas/api/backupcode/updatestatus';
		$customer_key = $mj_util->mo_jwt_get_option( 'mo_api_authentication_admin_customer_key' );
		$api_key      = $mj_util->mo_jwt_get_option( 'mo_api_authentication_admin_api_key' );
