...
class Feedback {
	...
	private function enqueue_styles() {
		...
		wp_enqueue_style( 'mo_jwt_feedback_style', MJ_URL . 'classes/Free/resources/feedback.css', array(), $ver = null, $in_footer = false );
	}
	/**
		...
		foreach ( $deactivate_reasons as $deactivate_reason ) {
			...
			<div type="radio" style="padding:1px;margin-left:2%;">
				<label style="font-weight:normal;font-size:14.6px" for="<?php echo esc_attr( $deactivate_reason); ?>">
					<input type="radio" style="display: inline-block;" name="mo_jwt_deactivate_reason_radio" value="<?php echo esc_attr( $deactivate_reason); ?>"
						required>
					<?php echo wp_kses( $deactivate_reason, \mo_jwt_get_valid_html() ); ?>
				</label>
			</div>
			<?php
		}
